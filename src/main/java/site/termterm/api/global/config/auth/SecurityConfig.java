...
public class SecurityConfig {
    ...
    @Bean
    // JWT 인증 방식을 사용할 예정이다. Session을 사용하지 않는다.
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception{

        httpSecurity.headers().frameOptions().disable();    // iframe을 허용하지 않는다

        httpSecurity.csrf().disable();      // csrf가 enable이면 postman이 작동하지 않는다.
        httpSecurity.cors().configurationSource(null);      // 일단 null이라고 해놓는다. 나중에 설정할 예정
        ...
        // httpBasic은 브라우저 팝업창을 이용해서 사용자 인증을 진행한다.
        ...
        // Exception 가로채기
        /* 인증 or 권한 관련된 오류가 나면, spring security는 필터에서 가로채서 처리를 한다. 이 제어권을 우리가 뺏어온다 */
        // 1. 인증실패
    ...
    public CorsConfigurationSource configurationSource(){
        ...
        configuration.addAllowedOriginPattern("*");     // 모든 IP 주소 허용  (나중에 프론트엔드 IP만 허용)  + 앱은 모든 사용자들이 다른 IP를 쓰기 때문에 어떻게 해줄 수가 없다. CORS에 걸리지도 않는다.
        configuration.setAllowCredentials(true);        // Client에서 Cookie 요청 허용
